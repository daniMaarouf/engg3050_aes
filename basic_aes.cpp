#include <stdio.h>
#include <stdint.h>

typedef uint32_t u32;

void key_expansion_core(unsigned char in[4], unsigned char i)
{
    const unsigned char rcon[11] = 
    {
        0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
    };
    
    const unsigned char s[256] = 
    {
        
    };

    /* shift rows */
    unsigned char temp = in[0];
    in[0] = in[1];
    in[1] = in[2];
    in[2] = in[3];
    in[3] = temp;

    /* S box */
    in[0] = s[in[0]];
    in[1] = s[in[1]];
    in[2] = s[in[2]];
    in[3] = s[in[3]];

    /* R con */
    in[0] ^= rcon[i];

}

void key_expansion(const unsigned char inputKey[32], unsigned char expanded_key[240]) 
{ 
    const unsigned char s[256] = 
    {
        
    };

    *((u32 *) expanded_key) = *((u32 *) inputKey);
    *((u32 *) (expanded_key + 4)) = *((u32 *) (inputKey + 4));
    *((u32 *) (expanded_key + 8)) = *((u32 *) (inputKey + 8));
    *((u32 *) (expanded_key + 12)) = *((u32 *) (inputKey + 12));
    *((u32 *) (expanded_key + 16)) = *((u32 *) (inputKey + 16));
    *((u32 *) (expanded_key + 20)) = *((u32 *) (inputKey + 20));
    *((u32 *) (expanded_key + 24)) = *((u32 *) (inputKey + 24));
    *((u32 *) (expanded_key + 28)) = *((u32 *) (inputKey + 28));

    u32 temp;
    int rconIteration = 1;
    int i;

    for (i = 32; i < 224; i += 32) {
        temp = *((u32 *) (expanded_key + i - 4));
        key_expansion_core((unsigned char *) &temp, rconIteration++);
        *((u32 *) (expanded_key + i)) = *((u32 *) (expanded_key + i - 32)) ^ temp;

        temp = *((u32 *) (expanded_key + i));
        *((u32 *) (expanded_key + i + 4)) = *((u32 *) (expanded_key + i - 28)) ^ temp;

        temp = *((u32 *) (expanded_key + i + 4));
        *((u32 *) (expanded_key + i + 8)) = *((u32 *) (expanded_key + i - 24)) ^ temp;

        temp = *((u32 *) (expanded_key + i + 8));
        *((u32 *) (expanded_key + i + 12)) = *((u32 *) (expanded_key + i - 20)) ^ temp;

        temp = *((u32 *) (expanded_key + i + 12));
        temp = (s[(temp & 0xff000000) >> 24] << 24) 
            | (s[(temp & 0x00ff0000) >> 16] << 16) 
            | (s[(temp & 0x0000ff00) >> 8] << 8) 
            | (s[(temp & 0x000000ff)]);
        *((u32 *) (expanded_key + i + 16)) = *((u32 *) (expanded_key + i - 16)) ^ temp;

        temp = *((u32 *) (expanded_key + i + 16));
        *((u32 *) (expanded_key + i + 20)) = *((u32 *) (expanded_key + i - 12)) ^ temp;

        temp = *((u32 *) (expanded_key + i + 20));
        *((u32 *) (expanded_key + i + 24)) = *((u32 *) (expanded_key + i - 8)) ^ temp;

        temp = *((u32 *) (expanded_key + i + 24));
        *((u32 *) (expanded_key + i + 28)) = *((u32 *) (expanded_key + i - 4)) ^ temp;
    }

    temp = *((u32 *) (expanded_key + 220));
    key_expansion_core((unsigned char *) &temp, rconIteration++);
    *((u32 *) (expanded_key + 224)) = *((u32 *) (expanded_key + 192)) ^ temp;

    temp = *((u32 *) (expanded_key + 224));
    *((u32 *) (expanded_key + 228)) = *((u32 *) (expanded_key + 196)) ^ temp;

    temp = *((u32 *) (expanded_key + 228));
    *((u32 *) (expanded_key + 232)) = *((u32 *) (expanded_key + 200)) ^ temp;

    temp = *((u32 *) (expanded_key + 232));
    *((u32 *) (expanded_key + 236)) = *((u32 *) (expanded_key + 204)) ^ temp;
}


/* substitute bytes (confustion) */
void SubBytes(unsigned char state[16]) 
{
    const unsigned char s[256] = 
    {
        
    };
   
    state[0] = s[state[0]];
    state[1] = s[state[1]];
    state[2] = s[state[2]];
    state[3] = s[state[3]];
    state[4] = s[state[4]];
    state[5] = s[state[5]];
    state[6] = s[state[6]];
    state[7] = s[state[7]];
    state[8] = s[state[8]];
    state[9] = s[state[9]];
    state[10] = s[state[10]];
    state[11] = s[state[11]];
    state[12] = s[state[12]];
    state[13] = s[state[13]];
    state[14] = s[state[14]];
    state[15] = s[state[15]];
}

void invSubBytes(unsigned char state[16])
{
    const unsigned char inv_s[256] = 
    {
        
    };

    state[0] = inv_s[state[0]];
    state[1] = inv_s[state[1]];
    state[2] = inv_s[state[2]];
    state[3] = inv_s[state[3]];
    state[4] = inv_s[state[4]];
    state[5] = inv_s[state[5]];
    state[6] = inv_s[state[6]];
    state[7] = inv_s[state[7]];
    state[8] = inv_s[state[8]];
    state[9] = inv_s[state[9]];
    state[10] = inv_s[state[10]];
    state[11] = inv_s[state[11]];
    state[12] = inv_s[state[12]];
    state[13] = inv_s[state[13]];
    state[14] = inv_s[state[14]];
    state[15] = inv_s[state[15]];
}

/* permute rows of state */
void ShiftRows(unsigned char state[16]) 
{ 
    unsigned char tmp[16];
    tmp[0] = state[0];
    tmp[1] = state[5];
    tmp[2] = state[10];
    tmp[3] = state[15];
    tmp[4] = state[4];
    tmp[5] = state[9];
    tmp[6] = state[14];
    tmp[7] = state[3];
    tmp[8] = state[8];
    tmp[9] = state[13];
    tmp[10] = state[2];
    tmp[11] = state[7];
    tmp[12] = state[12];
    tmp[13] = state[1];
    tmp[14] = state[6];
    tmp[15] = state[11];

    *((u32 *) state) = *((u32 *) tmp);
    *((u32 *) (state + 4)) = *((u32 *) (tmp + 4));
    *((u32 *) (state + 8)) = *((u32 *) (tmp + 8));
    *((u32 *) (state + 12)) = *((u32 *) (tmp + 12));
}

void invShiftRows(unsigned char state[16])
{
    unsigned char tmp[16];
    tmp[0] = state[0];
    tmp[1] = state[13];
    tmp[2] = state[10];
    tmp[3] = state[7];
    tmp[4] = state[4];
    tmp[5] = state[1];
    tmp[6] = state[14];
    tmp[7] = state[11];
    tmp[8] = state[8];
    tmp[9] = state[5];
    tmp[10] = state[2];
    tmp[11] = state[15];
    tmp[12] = state[12];
    tmp[13] = state[9];
    tmp[14] = state[6];
    tmp[15] = state[3];

    *((u32 *) state) = *((u32 *) tmp);
    *((u32 *) (state + 4)) = *((u32 *) (tmp + 4));
    *((u32 *) (state + 8)) = *((u32 *) (tmp + 8));
    *((u32 *) (state + 12)) = *((u32 *) (tmp + 12));
}

void MixColumns(unsigned char state[16]) 
{
    const unsigned char mul2[256] = 
    {
        
    };

    const unsigned char mul3[256] = 
    {
        
    };

    unsigned char tmp[16];
    tmp[0] = mul2[state[0]] ^ mul3[state[1]] ^ state[2] ^ state[3];
    tmp[1] = state[0] ^ mul2[state[1]] ^ mul3[state[2]] ^ state[3];
    tmp[2] = state[0] ^ state[1] ^ mul2[state[2]] ^ mul3[state[3]];
    tmp[3] = mul3[state[0]] ^ state[1] ^ state[2] ^ mul2[state[3]];

    tmp[4] = mul2[state[4]] ^ mul3[state[5]] ^ state[6] ^ state[7];
    tmp[5] = state[4] ^ mul2[state[5]] ^ mul3[state[6]] ^ state[7];
    tmp[6] = state[4] ^ state[5] ^ mul2[state[6]] ^ mul3[state[7]];
    tmp[7] = mul3[state[4]] ^ state[5] ^ state[6] ^ mul2[state[7]];

    tmp[8] = mul2[state[8]] ^ mul3[state[9]] ^ state[10] ^ state[11];
    tmp[9] = state[8] ^ mul2[state[9]] ^ mul3[state[10]] ^ state[11];
    tmp[10] = state[8] ^ state[9] ^ mul2[state[10]] ^ mul3[state[11]];
    tmp[11] = mul3[state[8]] ^ state[9] ^ state[10] ^ mul2[state[11]];

    tmp[12] = mul2[state[12]] ^ mul3[state[13]] ^ state[14] ^ state[15];
    tmp[13] = state[12] ^ mul2[state[13]] ^ mul3[state[14]] ^ state[15];
    tmp[14] = state[12] ^ state[13] ^ mul2[state[14]] ^ mul3[state[15]];
    tmp[15] = mul3[state[12]] ^ state[13] ^ state[14] ^ mul2[state[15]];

    *((u32 *) state) = *((u32 *) tmp);
    *((u32 *) (state + 4)) = *((u32 *) (tmp + 4));
    *((u32 *) (state + 8)) = *((u32 *) (tmp + 8));
    *((u32 *) (state + 12)) = *((u32 *) (tmp + 12));
}

/*
    0E 0B 0D 09
    09 0E 0B 0D
    0D 09 0E 0B
    0B 0D 09 0E
    14 11 13 09
    09 14 11 13
    13 09 14 11
    11 13 09 14
*/
void invMixColumns(unsigned char state[16])
{

    static const unsigned char mul9[256] = 
    {
        
    };

    static const unsigned char mul11[256] = 
    {
        
    };

    static const unsigned char mul13[256] = 
    {
        
    };

    static const unsigned char mul14[256] = 
    {
       
    };

    unsigned char tmp[16];

    tmp[0] = mul14[state[0]] ^ mul11[state[1]] ^ mul13[state[2]] ^ mul9[state[3]];
    tmp[1] = mul9[state[0]] ^ mul14[state[1]] ^ mul11[state[2]] ^ mul13[state[3]];
    tmp[2] = mul13[state[0]] ^ mul9[state[1]] ^ mul14[state[2]] ^ mul11[state[3]];
    tmp[3] = mul11[state[0]] ^ mul13[state[1]] ^ mul9[state[2]] ^ mul14[state[3]];

    tmp[4] = mul14[state[4]] ^ mul11[state[5]] ^ mul13[state[6]] ^ mul9[state[7]];
    tmp[5] = mul9[state[4]] ^ mul14[state[5]] ^ mul11[state[6]] ^ mul13[state[7]];
    tmp[6] = mul13[state[4]] ^ mul9[state[5]] ^ mul14[state[6]] ^ mul11[state[7]];
    tmp[7] = mul11[state[4]] ^ mul13[state[5]] ^ mul9[state[6]] ^ mul14[state[7]];

    tmp[8] = mul14[state[8]] ^ mul11[state[9]] ^ mul13[state[10]] ^ mul9[state[11]];
    tmp[9] = mul9[state[8]] ^ mul14[state[9]] ^ mul11[state[10]] ^ mul13[state[11]];
    tmp[10] = mul13[state[8]] ^ mul9[state[9]] ^ mul14[state[10]] ^ mul11[state[11]];
    tmp[11] = mul11[state[8]] ^ mul13[state[9]] ^ mul9[state[10]] ^ mul14[state[11]];

    tmp[12] = mul14[state[12]] ^ mul11[state[13]] ^ mul13[state[14]] ^ mul9[state[15]];
    tmp[13] = mul9[state[12]] ^ mul14[state[13]] ^ mul11[state[14]] ^ mul13[state[15]];
    tmp[14] = mul13[state[12]] ^ mul9[state[13]] ^ mul14[state[14]] ^ mul11[state[15]];
    tmp[15] = mul11[state[12]] ^ mul13[state[13]] ^ mul9[state[14]] ^ mul14[state[15]];

    *((u32 *) state) = *((u32 *) tmp);
    *((u32 *) (state + 4)) = *((u32 *) (tmp + 4));
    *((u32 *) (state + 8)) = *((u32 *) (tmp + 8));
    *((u32 *) (state + 12)) = *((u32 *) (tmp + 12));
}

/* galois addition */
void AddRoundKey(unsigned char state[16], const unsigned char roundKey[16])
{
    *((u32 *) state) ^= *((u32 *) roundKey);
    *((u32 *) (state + 4)) ^= *((u32 *) (roundKey + 4));
    *((u32 *) (state + 8)) ^= *((u32 *) (roundKey + 8));
    *((u32 *) (state + 12)) ^= *((u32 *) (roundKey + 12));
}

void AES_Encrypt(const unsigned char message_in[16], unsigned char message_out[16], const unsigned char expanded_key[240])
{
    int i;
    unsigned char state[16];
    *((u32 *) state) = *((u32 *) message_in);
    *((u32 *) (state + 4)) = *((u32 *) (message_in + 4));
    *((u32 *) (state + 8)) = *((u32 *) (message_in + 8));
    *((u32 *) (state + 12)) = *((u32 *) (message_in + 12));
    
    AddRoundKey(state, expanded_key); /* whitening */

    Encrypt_Rounds_Loop: for (i = 1; i < 14; i++) {
        SubBytes(state);     /* confusion */
        ShiftRows(state);    /* permutation */
        MixColumns(state);   /* diffusion */
        AddRoundKey(state, expanded_key + (16 * i));
    }

    /* diffusion not needed since no more rounds will be performed */
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, expanded_key + (16 * 14));

    *((u32 *) message_out) = *((u32 *) state);
    *((u32 *) (message_out + 4)) = *((u32 *) (state + 4));
    *((u32 *) (message_out + 8)) = *((u32 *) (state + 8));
    *((u32 *) (message_out + 12)) = *((u32 *) (state + 12));
}

void AES_Decrypt(const unsigned char message_in[16], unsigned char message_out[16], const unsigned char expanded_key[240])
{
    unsigned char state[16]; 
    *((u32 *) state) = *((u32 *) message_in);
    *((u32 *) (state + 4)) = *((u32 *) (message_in + 4));
    *((u32 *) (state + 8)) = *((u32 *) (message_in + 8));
    *((u32 *) (state + 12)) = *((u32 *) (message_in + 12));

    int i;

    AddRoundKey(state, expanded_key + (16 * 14));
    invShiftRows(state);
    invSubBytes(state);

    Decrypt_Rounds_Loop: for (i = 13; i >= 1; i--) {
        AddRoundKey(state, expanded_key + (16 * i));
        invMixColumns(state);
        invShiftRows(state);
        invSubBytes(state);
    }

    AddRoundKey(state, expanded_key); /* whitening */

    *((u32 *) message_out) = *((u32 *) state);
    *((u32 *) (message_out + 4)) = *((u32 *) (state + 4));
    *((u32 *) (message_out + 8)) = *((u32 *) (state + 8));
    *((u32 *) (message_out + 12)) = *((u32 *) (state + 12));
}

